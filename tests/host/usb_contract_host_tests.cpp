...
void test_init_failure() noexcept
{
    callback_state callbacks{};
    host_test::fake_usb::reset();
...
void test_invalid_write() noexcept
{
    ...
    initialize(callbacks);
    connect();
    std::array<std::uint8_t, 129> too_large{};
...
void test_capabilities() noexcept
{
    ...
    require(capabilities.max_transfer_size == 128U);
...
int main(int argc, char** argv)
{
    ...
    if (scenario == "init_once") test_init_once();
    else if (scenario == "init_failure") test_init_failure();
